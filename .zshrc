...
if [[ $UID -ge 1000 && -d ~/ownCloud/.bin && -z $(echo $PATH | grep -o ~/ownCloud/.bin) ]]; then
...
#   for i in {0..255}; do
#     local pattern=$((-i%6))
#     local newline=${${(M)pattern:#3}:+$'\n'}
#     printf -v padded '%03d' $i
#     printf -v color '\e[48;5;%sm' $i
#     printf '%s%3s%s ' "${color}" "${padded}" "${reset-color}"
#     printf '%s' "$newline"
#   done
# }
